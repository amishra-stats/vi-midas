data {
// here we have implemented the model estimation on the  training data [without geochemical  component]
...
model {
  ...
  for(i in 1:n){
    ...
    for(j in 1:q){
        if (holdout[i,j] == 0){
            // mean parameter
            mu[j] = C0[j] + S[i]*(A_s*(L_sp[j,]')) + B[i]*(A_b*(L_sp[j,]')) + Q[i]*(A_m*(L_sp[j,]'));
            if (Yi[i,j] != 0){ 
