data {
// here we have implemented the model estimation on the  training data [without quarter/time/season component]
...
model {
  ...
  for(i in 1:n){
    ...
    for(j in 1:q){
        if (holdout[i,j] == 0){
            # mean parameter 
            mu[j] = C0[j] + X[i]*C_geo[j] + S[i]*(A_s*(L_sp[j,]')) + B[i]*(A_b*(L_sp[j,]'));
            if (Yi[i,j] != 0){ 
